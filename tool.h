...
template <> void buffer::put<uint16_t>(uint16_t a);

template <> void buffer::put<uint32_t>(uint32_t a);

template <> uint16_t buffer::get<uint16_t>() const;

template <> uint32_t buffer::get<uint32_t>() const;
...
template <class T> writer<T>& get_writer(T& value)
{
