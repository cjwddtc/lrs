...
namespace lua
{
    ...
    class signal
    {
        typedef boost::variant< std::pair< lua_State*, int >,
                                std::function< void() > >
            function;
    ...
    std::vector< std::pair< std::string, lua_CFunction > > reg_funcs
        = {std::make_pair("resign", lua_resign),
           std::make_pair("trigger", lua_trigger),
           std::make_pair("test", lua_test)};
    ...
    void add_data(void* ptr, std::string name,
    ...
    void signal::function_run::
    operator()(std::pair< lua_State*, int > func) const
    {
