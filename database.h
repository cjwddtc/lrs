...
class database
{
  ...
  public:
	...
	template <class T> void async_run(std::function<void(T)> func)
	{
	    io.post([func, this]() {
...
template <class T> class bind_base
{
  public:
	...
	boost::
	    variant<proxy<int> T::*, proxy<int> T::*, proxy<std::string> T::*>
	        value;
	std::string name;
    };
    ...
    template <class F> void add(proxy<F> T::*a, const std::string& name)
    {
	auto p = name.find('.');
	assert(p != std::string::npos);
	values.insert(std::make_pair(name.substr(0, p),
