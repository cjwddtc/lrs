...
std::string wrap_type(const std::string& type_name, const std::string& value_template)
{
...
int main(int argv, char* args[])
{
	...
	for(auto& b : a.second)
	    vec.back().second.push_back(std::make_pair(b.first, b.second.data()));
    }
    if(strcmp(args[1],"-h")==0){
	    ...
	    for(auto& a : vec) {
			...
			for(auto& b : a.second) {
				if(flag)
					std::cout<<',';
				std::cout<< '.'<<b.first<<'=' <<"db_attribute(\""<< a.first <<"\",\"" <<  b.first << "\")";
				flag=true;
			}
			std::cout << "};\n";
