...
p=new_statement("CREATE TABLE player(name varchar,passwd varchar,is_valid bool)");
p->bind([p](bool flag){assert(flag);delete p;});
}
{
...
p->bind([p](bool flag){assert(flag);delete p;});
p=new_statement("CREATE TABLE room_contain(room_name varchar,rolename varchar,count size)");
p->bind([p](bool flag){assert(flag);delete p;});
}
{
...
p->bind([p](bool flag){assert(flag);delete p;});
p=new_statement("CREATE TABLE score(name varchar,room_name varchar,score int)");
p->bind([p](bool flag){assert(flag);delete p;});
}
}
}
...
,get_room_size(this,"select sum(count) from room_contain where room_name=?") 
