...
void lsy::run_thread::run()
{
	boost::thread([this]() {
		...
		work->get_io_service().run(); }).swap(thr);
}
...
void lsy::run_thread::add_room(std::string rule_name_, std::vector<lsy::role_info> vec)
{
	work->get_io_service().post([rule_name_, vec = std::move(vec)]() {new lsy::room(rule_name_, vec); });
}
...
void lsy::server::create_room(std::string rule_name_, std::vector<lsy::role_info> vec)
{
